...
class ResizeImage {
     ...
     * @throws Exception
     */
    protected function setImage(string $filename): void
    {
        $size = getimagesize($filename);
        $this->ext = $size['mime'];

        $this->image = match ($this->ext) {
            ...
            default => throw new Exception("File is not an image, please use another file type.", 1),
        };
     ...
     */
    public function resizeTo(int $width, int $height, string $resizeOption = 'default'): void
    {
     ...
     * @return float
     */
    protected function resizeHeightByWidth(int $width): float
    {
     ...
     * @return float
     */
    protected function resizeWidthByHeight(int $height): float
    {
